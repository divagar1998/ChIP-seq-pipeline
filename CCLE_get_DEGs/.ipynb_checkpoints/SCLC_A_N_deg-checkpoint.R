...
model_csv <- read.csv("/hpc/users/divagt01/watanabe/Divagar/CCLE_data/sclc_model.csv")
tpm_csv <- read.csv("/hpc/users/divagt01/watanabe/Divagar/CCLE_data/sclc_tpm.csv")
...
EnhancedVolcano(top_table1,
                lab = top_table1$ID,           # Labels for each point (gene names)
                x = 'logFC',                  # Log2 Fold Change for X-axis
                y = 'adj.P.Val',              # Adjusted P-Value for Y-axis
                pCutoff = 0.01,               # Significance cutoff for p-value
                FCcutoff = 2,                 # Log2 fold change cutoff
                pointSize = 3.0,              # Size of points
                colAlpha = 0.6,               # Transparency of points
                legendPosition = 'topright',  # Position of the legend
                legendLabSize = 12,           # Size of legend labels
                axisLabSize = 14,             # Size of axis labels
                title = "DEGs of SCLC-A compared to SCLC-N", # Plot title
                drawConnectors = TRUE,        # Add lines connecting points to labels
                labSize = 5.0,
                max.overlaps = 20,
                subtitle = '',
...
EnhancedVolcano(top_table2,
                lab = top_table2$ID,           # Labels for each point (gene names)
                x = 'logFC',                  # Log2 Fold Change for X-axis
                y = 'adj.P.Val',              # Adjusted P-Value for Y-axis
                pCutoff = 0.01,               # Significance cutoff for p-value
                FCcutoff = 2,                 # Log2 fold change cutoff
                ...
                legendLabSize = 12,           # Size of legend labels
                axisLabSize = 14,             # Size of axis labels
                title = "DEGs of SCLC-A compared to SCLC-P", # Plot title
                drawConnectors = TRUE,        # Add lines connecting points to labels
                labSize = 6.0,
                boxedLabels=TRUE,
...
EnhancedVolcano(top_table3,
                lab = top_table3$ID,           # Labels for each point (gene names)
                x = 'logFC',                  # Log2 Fold Change for X-axis
                y = 'adj.P.Val',              # Adjusted P-Value for Y-axis
                pCutoff = 0.01,               # Significance cutoff for p-value
                FCcutoff = 2,                 # Log2 fold change cutoff
                ...
                legendLabSize = 12,           # Size of legend labels
                axisLabSize = 14,             # Size of axis labels
                title = "DEGs of SCLC-N compared to SCLC-P", # Plot title
                drawConnectors = TRUE,        # Add lines connecting points to labels
                labSize = 6.0,
                max.overlaps = 20,
                boxedLabels=TRUE,
